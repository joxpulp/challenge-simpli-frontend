...
function Accessories({ products, totalPages, currentPage, limit }: ProductsPageSSRProps) {
  return (
    <ProductContainer>
      <ProductContainerGrid>
        {products.map(({ _id, image, name, price, currency, slug }) => (
          <ProductCard key={_id} imageUrl={image} title={name} subtitle={`$${price} ${currency}`} urlToGo={`/accessories/${slug}`} />
        ))}
      </ProductContainerGrid>
...
export async function getServerSideProps({ query }: GetServerSidePropsContext) {
  ...
  } = await api.get<ProductResponse>(`/accessories/list?page=${page}&limit=${limit}`);
